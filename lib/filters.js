...
function matchesQuery(text, query) {
  ...
  return groups.some((group) => group.every((word) => normalizedText.includes(word)));
}
...
function filterDecision(ad, configInput) {
  ...
  if (config.query && !matchesQuery(text, config.query)) return { match: false, detailsUseful: false, reason: `нет ключевых слов: ${config.query}` };
