...
function filterDecision(ad, configInput) {
  ...
  if (missingStrictValue(price, config.priceMin, config.priceMax)) return { match: false, detailsUseful: true };
  if (!inRange(price, config.priceMin, config.priceMax, true)) return { match: false, detailsUseful: false };
  if (missingStrictValue(areas.totalArea, config.totalAreaMin, config.totalAreaMax)) return { match: false, detailsUseful: true };
  if (!inRange(areas.totalArea, config.totalAreaMin, config.totalAreaMax, true)) return { match: false, detailsUseful: false };
  if (ad.propertyType === 'room' && missingStrictValue(areas.roomArea, config.roomAreaMin, config.roomAreaMax)) return { match: false, detailsUseful: true };
  if (ad.propertyType === 'room' && !inRange(areas.roomArea, config.roomAreaMin, config.roomAreaMax, true)) return { match: false, detailsUseful: false };
  if (ad.propertyType === 'flat' && config.rooms.length && rooms === null) return { match: false, detailsUseful: true };
  if (ad.propertyType === 'flat' && config.rooms.length && !config.rooms.includes(rooms)) return { match: false, detailsUseful: false };
  if (!inRange(buildYear, config.buildYearMin, config.buildYearMax, false)) return { match: false, detailsUseful: false };
  if (!inRange(floorInfo.floor, config.floorMin, config.floorMax, false)) return { match: false, detailsUseful: false };
  if (!inRange(floorInfo.floorsTotal, config.floorsTotalMin, config.floorsTotalMax, false)) return { match: false, detailsUseful: false };
  if (!inRange(metro.minutes, config.metroMinutesMin, config.metroMinutesMax, false)) return { match: false, detailsUseful: false };
  if (config.metroMode !== 'any' && metro.mode && metro.mode !== config.metroMode) return { match: false, detailsUseful: false };
  if (config.sellerType !== 'any' && sellerType === null) return { match: false, detailsUseful: true };
  if (config.sellerType !== 'any' && sellerType !== config.sellerType) return { match: false, detailsUseful: false };
  if (config.deposit !== 'any' && deposit === null) return { match: false, detailsUseful: true };
  if (config.deposit !== 'any' && deposit !== config.deposit) return { match: false, detailsUseful: false };
  if (config.query && !text.toLowerCase().includes(config.query.toLowerCase())) return { match: false, detailsUseful: false };
  return { match: true, detailsUseful: false };
}
