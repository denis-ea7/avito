...
function extractMetro(text) {
  const normalized = normalizeText(text)
    ...
    .toLowerCase();
  const matches = [];
  const pattern = /(?:~\s*)?(?:(\d{1,2})\s*-\s*(\d{1,2})|от\s*(\d{1,2})|(\d{1,2}))\s*мин(?:ут[аы]?|\.|)?(?:\s*(пешком|на транспорте|транспортом|транспорт|🚶|🚌|🚇))?/gi;
  for (const match of normalized.matchAll(pattern)) {
    const raw = match[0] || '';
    const start = match.index || 0;
    const around = normalized.slice(Math.max(0, start - 40), Math.min(normalized.length, start + raw.length + 40));
    if (/назад/.test(around)) continue;
    const values = [match[1], match[2], match[3], match[4]].filter(Boolean).map(Number).filter((value) => Number.isFinite(value));
    if (!values.length) continue;
    const minutes = Math.max(...values);
    const modeText = [match[5], raw, around].filter(Boolean).join(' ');
    const mode = /пешком|🚶/.test(modeText) ? 'foot' : /транспорт|🚌/.test(modeText) ? 'transport' : 'foot';
    matches.push({ minutes, mode });
  }
  if (!matches.length) return { minutes: null, mode: null };
  const footMatches = matches.filter((item) => item.mode === 'foot');
  const transportMatches = matches.filter((item) => item.mode === 'transport');
  const preferred = footMatches.length ? footMatches : transportMatches.length ? transportMatches : matches;
  return preferred.sort((left, right) => left.minutes - right.minutes)[0];
}
...
function extractDeposit(text) {
  const normalized = normalizeText(text).toLowerCase();
  if (/(?:без залога|без депозита|залог не требуется|депозит не требуется|(?:залог|депозит)\s*[:\-–—]?\s*0(?:\D|$))/i.test(normalized)) return 'no';
...
function filterDecision(ad, configInput) {
  ...
  if ((config.metroMinutesMin !== '' || config.metroMinutesMax !== '') && metro.minutes === null) return { match: false, detailsUseful: true, reason: 'метро не найдено' };
  if (!inRange(metro.minutes, config.metroMinutesMin, config.metroMinutesMax, true)) return { match: false, detailsUseful: false, reason: `метро ${metro.minutes ?? 'не найдено'} мин вне диапазона` };
  if (config.metroMode !== 'any' && !metro.mode) return { match: false, detailsUseful: true, reason: 'способ до метро не найден' };
